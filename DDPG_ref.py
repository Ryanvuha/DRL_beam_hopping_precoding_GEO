...
H_Chrl = Channel_Load1(strings)
T = H_Chrl.shape[2]
    
# Takes about 4 min to train

num_user = H_Chrl.shape[0]
num_beam = H_Chrl.shape[1]
...
class Buffer:
    ...
    def update(
        self, state_batch, action_batch, reward_batch, next_state_batch, \
            target_actor, target_critic, critic_model, actor_model, critic_optimizer, actor_optimizer, gamma
    ):
    ...
    def learn(self, target_actor, target_critic, critic_model, actor_model, critic_optimizer, actor_optimizer, gamma):
        # Get sampling range
...
def policy(state, noise, actor_model):
    sampled_actions = tf.squeeze(actor_model(state))
    #noise = noise_object()
    # Adding noise to action
    sampled_actions = sampled_actions.numpy() + noise
...
class model_DDPG():
    ...
    def train(self):
        self.buffer.learn(self.target_actor, self.target_critic, self.critic_model, self.actor_model, self.critic_optimizer, self.actor_optimizer, self.gamma)
    ...
    def update(self,prev_state, action, reward, state):
